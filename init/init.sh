...
-- User 3: dedicated Feast feature store user
CREATE USER "${FEAST_DB_USER}" WITH PASSWORD '${FEAST_DB_USER_PASSWORD}';
GRANT CONNECT ON DATABASE ${PG_SPOTIFY_DB} TO "${FEAST_DB_USER}";
ALTER ROLE "${FEAST_DB_USER}" SET search_path TO feast, staging, marts, raw;
